...
var waterlogImplementors = []string{"WaterLog", "Node"}

// IsNode implements the Node interface check for GQLGen.
func (*WaterLog) IsNode() {}
...
func (c *Client) noder(ctx context.Context, table string, id uuid.UUID) (Noder, error) {
	switch table {
	case waterlog.Table:
		query := c.WaterLog.Query().
			Where(waterlog.ID(id))
		if fc := graphql.GetFieldContext(ctx); fc != nil {
			if err := query.collectField(ctx, true, graphql.GetOperationContext(ctx), fc.Field, nil, waterlogImplementors...); err != nil {
				return nil, err
			}
		}
...
func (c *Client) noders(ctx context.Context, table string, ids []uuid.UUID) ([]Noder, error) {
	...
	case waterlog.Table:
		query := c.WaterLog.Query().
			Where(waterlog.IDIn(ids...))
		query, err := query.CollectFields(ctx, waterlogImplementors...)
		if err != nil {
			return nil, err
		}
